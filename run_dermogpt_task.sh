...
# Image resolution constraints for Qwen2.5-VL (controls token count per image)
...
export PYTHONPATH="/fs04/scratch2/ub62/ssim0070/python_pkgs/vllm_016_native:${WORK_DIR}:${PYTHONPATH}"
...
# We use a standalone script to avoid shell escaping issues and respect pixel constraints
echo "Performing Vision-Aware prompt length check (Limit: ${MAX_PROMPT_LEN} tokens, MaxPixels: ${MAX_PIXELS})..."
...
def filter_dataset(input_path, output_path, model_path, max_len, max_pixels, min_pixels):
    ...
    # Apply pixel constraints to the processor
...
if __name__ == '__main__':
    # Args: model_path, max_len, max_pixels, min_pixels, in_train, out_train, [in_val, out_val, ...]
    ...
    for i in range(0, len(paths), 2):
        filter_dataset(paths[i], paths[i+1], model_path, max_len, max_pixels, min_pixels)
EOF
...
apptainer exec --nv --bind /fs04/scratch2/ub62/ssim0070:/fs04/scratch2/ub62/ssim0070 "${APPTAINER_IMG}" \
    ...
    "${MODEL_PATH}" "${MAX_PROMPT_LEN}" "${MAX_PIXELS}" "${MIN_PIXELS}" \
