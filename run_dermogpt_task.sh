...
APPTAINER_IMG="/fs04/scratch2/ub62/ssim0070/verl_vllm011.latest.sif"
...
# This is CPU-safe and does not require downloading model weights.
try:
    ...
    class _DummyConfig:
        ...
        sliding_window = None
        max_window_layers = 0
        # Valid mrope_section list (sum=4 -> repeated sum=8) for head_dim=8 dummy test
...
try:
    ...
    # Older processors may not accept pixel kwargs at init; fall back.
    processor = AutoProcessor.from_pretrained(model_path, trust_remote_code=True)

# Full model forward is only reasonable when GPU is visible.
if device == "cuda":
    ...
    # Tiny dummy image; the goal is to exercise vision + attention paths.
    img = Image.new("RGB", (224, 224), color=(127, 127, 127))
    user_text = "Describe the image in one sentence."

    # Qwen-VL models require explicit image placeholder tokens in the prompt.
